...
public class UserTestCasesForSearch implements
  ...
  public Stream<TestTemplateInvocationContext> provideTestTemplateInvocationContexts(ExtensionContext context) {
    ...
    /*
     * [TC_ID : TC-002] [TC명 : 사용자 정보 상세조회1] [REQ_ID : REQ_ADM_002] [화면 : 조직 관리 >
     * 사용자 관리] [기능 : 사용자 정보 상세조회] [테스트항목 : [단건] 정상 조회] [테스트 상세 : ]
     */

    String td1 = "[JUnit]admin001";
    UserResDto ed1 = new UserResDto().builder()
    ...
    testCases.add(
        new TestCaseDetail<>(BASE_URL + "/[JUnit]admin001", "정상 조회", td1,
            ResponseApi.success(Map.of("data", ed1)), status().isOk()));

    /*
     * [TC_ID : TC-003] [TC명 : 사용자 정보 상세조회2] [REQ_ID : REQ_ADM_002] [화면 : 조직 관리 >
     * 사용자 관리] [기능 : 사용자 정보 상세조회] [테스트항목 : [단건] 필수값 누락] [테스트 상세 : ]
     */
    // 해당 케이스 미존재
    testCases.add(new TestCaseDetail<>("N/A", "필수값 누락", null, null, status().isOk()));

    /*
     * [TC_ID : TC-004] [TC명 : 사용자 정보 상세조회3] [REQ_ID : REQ_ADM_002] [화면 : 조직 관리 >
     * 사용자 관리] [기능 : 사용자 정보 상세조회] [테스트항목 : [단건] 존재하지 않는 ID 조회] [테스트 상세 : ]
     */
    testCases.add(
        new TestCaseDetail<>(BASE_URL + "/[JUnit]admin001_NOT_EXIST", "존재하지 않는 ID 조회", "admin001_NOT_EXIST",
            ResponseApi.fail(ErrorCode.OBJECT_NOT_FOUND), status().is4xxClientError()));
    /*
     * [TC_ID : TC-005] [TC명 : 사용자 정보 상세조회4] [REQ_ID : REQ_ADM_002] [화면 : 조직 관리 >
     * 사용자 관리] [기능 : 사용자 정보 상세조회] [테스트항목 : [단건] 비즈니스로직 케이스] [테스트 상세 : ]
     */
    // 해당 케이스 미존재
    testCases.add(new TestCaseDetail<>("N/A", "비즈니스로직 케이스", null, null, status().isOk()));

    /*
     * [TC_ID : TC-006] [TC명 : 사용자 정보 상세조회5] [REQ_ID : REQ_ADM_002] [화면 : 조직 관리 >
     * 사용자 관리] [기능 : 사용자 정보 상세조회] [테스트항목 : 잘못된 형식 입력] [테스트 상세 : ]
     */
    testCases.add(
    ...
    return testCases.stream().map(testCase -> new TestTemplateInvocationContext() {
      ...
      public String getDisplayName(int invocationIndex) {
        return "[" + invocationIndex + "] " + "사용자 조회 : " + (testCase.getTestName());
