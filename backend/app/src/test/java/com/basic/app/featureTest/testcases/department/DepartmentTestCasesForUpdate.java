...
public class DepartmentTestCasesForUpdate implements TestTemplateInvocationContextProvider {
    ...
    public Stream<TestTemplateInvocationContext> provideTestTemplateInvocationContexts(ExtensionContext context) {
         ...
         * 관리] [기능 : 부서 정보 추가] [테스트항목 : 비즈니스로직 케이스] [테스트 상세 : 등록불가]
         */

        String url_order1 = BASE_URL;
        String testName_order1 = "비즈니스로직 케이스 | 수정불가";
        ...
        return testCases.stream().map(testCase -> new TestTemplateInvocationContext() {
            ...
            public String getDisplayName(int invocationIndex) {
                return "[" + invocationIndex + "] " + "부서 수정 : " + (testCase.getTestName());
            }
