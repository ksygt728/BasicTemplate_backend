...
public class ChaebunTestCasesForUpdate implements TestTemplateInvocationContextProvider {
  ...
  public Stream<TestTemplateInvocationContext> provideTestTemplateInvocationContexts(ExtensionContext context) {
    ...
    /*
     * [TC_ID : TC-267] [TC명 : 채번 수정1] [REQ_ID : REQ_ADM_055] [화면 : 시스템 관리 > 채번관리]
     * [기능 : 채번 수정] [테스트항목 : 정상 수정] [테스트 상세 : ]
     */
    ChaebunReqDto td1 = ChaebunReqDto.builder()
        .seqId("[JUnit]SEQ002")
        .seqName("[JUnit]주문번호채번_수정")
        .pattern("{PREFIX}_{DATEFORMAT}_{VALUE}_UPD")
        .prefix("[JUnit]ORDER_UPD")
        .step(2)
        .length(8)
    ...
    /*
     * [TC_ID : TC-268] [TC명 : 채번 수정2] [REQ_ID : REQ_ADM_055] [화면 : 시스템 관리 > 채번관리]
     * [기능 : 채번 수정] [테스트항목 : 필수값 누락] [테스트 상세 : ]
     */

    // 채번아이디 누락
    ChaebunReqDto td2_1 = ChaebunReqDto.builder()
        .seqId("") // 채번아이디 누락
        .seqName("[JUnit]아이디누락테스트")
    ...
    testCases.add(
        new TestCaseDetail<>(BASE_URL, "필수값 누락 - 채번아이디", td2_1,
    ...
    testCases.add(
        new TestCaseDetail<>(BASE_URL, "필수값 누락 - 채번명", td2_2,
    ...
    testCases.add(
        new TestCaseDetail<>(BASE_URL, "필수값 누락 - 채번패턴", td2_3,
    ...
    testCases.add(
        new TestCaseDetail<>(BASE_URL, "필수값 누락 - 채번고유번호", td2_4,
    ...
    testCases.add(
        new TestCaseDetail<>(BASE_URL, "필수값 누락 - 채번길이", td2_6,
            ResponseApi.fail(ErrorCode.VALIDATION_ERROR_CLIENT, "채번길이는 1 이상이어야 합니다."), status().is4xxClientError()));

    /*
     * [TC_ID : TC-269] [TC명 : 채번 수정3] [REQ_ID : REQ_ADM_055] [화면 : 시스템 관리 > 채번관리]
     * [기능 : 채번 수정] [테스트항목 : 존재하지 않는 ID 수정 시도] [테스트 상세 : ]
     */
    ChaebunReqDto td3 = ChaebunReqDto.builder()
        .seqId("[JUnit]SEQ_NOT_EXIST")
        .seqName("[JUnit]존재하지않는데이터수정")
        .pattern("{PREFIX}_{VALUE}")
        .prefix("[JUnit]NOTEXIST")
        .step(1)
        .length(4)
        .build();
    ...
    /*
     * [TC_ID : TC-270] [TC명 : 채번 수정4] [REQ_ID : REQ_ADM_055] [화면 : 시스템 관리 > 채번관리]
     * [기능 : 채번 수정] [테스트항목 : 비즈니스로직 케이스] [테스트 상세 : ]
     */
    // 해당 케이스 미존재
    testCases.add(new TestCaseDetail<>("N/A", "비즈니스로직 케이스", null, null, status().isOk()));

    /*
     * [TC_ID : TC-271] [TC명 : 채번 수정5] [REQ_ID : REQ_ADM_055] [화면 : 시스템 관리 > 채번관리]
     * [기능 : 채번 수정] [테스트항목 : 잘못된 형식 입력] [테스트 상세 : ]
     */
    ...
    return testCases.stream().map(testCase -> new TestTemplateInvocationContext() {
      ...
      public String getDisplayName(int invocationIndex) {
        return "[" + invocationIndex + "] " + "채번 수정 : " + (testCase.getTestName());
      }
