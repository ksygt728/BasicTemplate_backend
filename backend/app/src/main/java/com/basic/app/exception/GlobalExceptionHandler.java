 ...
 *     - 예외상황이 명확함
 *     2) [중] ClientActionException : 비즈니스예외는 아니지만 명확한 클라이언트의 잘못된 동작으로 인해 예외가
 *     발생하는 경우
 *     3) [상] SystemErrorException : try-catch 등 예외가 발생하는 구간이 있지만 어떤 예외가 발생할지
 *     애매한경우(예측불가) 각종 에러가 발생할 수 있는 부분 등 예외함.
 ...
 *     - 여기서 예외가 잡혔다면 명확한 처리를 위해 별도로 실제 예외를 던져야함(BusinessException 또는
...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ResponseApi<?>> handleBusinessException(BusinessException e, HttpServletRequest request) {
    ...
    String locale = request.getHeader("X-Lang");
    String message = messageSource.getMessage("err." + e.getErrorCode().getCode(), locale, null);

    showErrorLogFormat(e, errorCode, message, additionalMessage);
    ...
    return ResponseEntity
        ...
        .body(ResponseApi.fail(errorCode, message, additionalMessage));
  }
  ...
  private void showErrorLogFormat(Exception e, ErrorCode errorCode, String message, String additionalMessage) {
    log.warn(
        """
