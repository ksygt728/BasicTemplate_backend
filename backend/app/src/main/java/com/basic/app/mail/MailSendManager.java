 ...
 * @설명 : 실제 메일 발송 클래스
 *     [1. 메일을 서비스단에서 직접 실시간으로 보내는 경우] : 서비스단에서 비즈니스처리와 동시에 실시간으로 메일을 보내야 하는 경우
 *     사용. MailH에 이력을 남김 ==> 메일을 보내고 이력저장
 *     [2. 메일을 배치로 보내는 경우] : 서비스단에서는 MailH에 이력만 남겨놓고 quartz 배치에서 주기적으로 메일을 전송하는
 *     경우 사용. MailH에 이력을 남김 ==> 이력을 남기고 보냄
 *     [3. 메일 이력을 직접 실행해서 전송하는 경우] : 전송또는 미전송 된 메일을 재전송하고 싶은경우 사용(관리자). MailH에
 *     이력을 남김 ==> 이력을 남기고 보냄
 *     [4. 메일을 이력에 쌓아놓고 배치주기로 메일 전송] : 2번을 수행하기 위해 메일을 insert함. 이력을 남기고 보내는 로직이기
 *     떄문에, 직접 에러를 return ==> 이력을 남기고 보내는데 그 자체가 오류라면 의미가 없어서 직접 return
 * @작성자 : 김승연
 * @작성일 : 2025.08.19
 * @변경이력 :
